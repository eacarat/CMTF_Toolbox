function [f,G] = acmtf_fg(Z,A,Znormsqr, beta)
...
for p = 1:P   
    ...
    if length(size(Z.object{p}))>=3
        ...
        if isfield(Z,'miss') && ~isempty(Z.miss{p})            
            [fp{p},Gp{p}] = scp_wfg(Z.object{p}, Z.miss{p}, B, Znormsqr{p}, beta(p));        
        else
            [fp{p},Gp{p}] = scp_fg(Z.object{p}, B, Znormsqr{p}, beta(p));        
        end
    elseif length(size(Z.object{p}))==2
        % Matrix
        if isfield(Z,'miss') && ~isempty(Z.miss{p})
            [fp{p},Gp{p}] = spca_wfg(Z.object{p}, Z.miss{p}, B, Znormsqr{p}, beta(p));                        
        else
            [fp{p},Gp{p}] = spca_fg(Z.object{p}, B, Znormsqr{p}, beta(p));                         
        end
    end
    G.norms(p) = Gp{p}(end);    
end
