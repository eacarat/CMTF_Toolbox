function [f,G] = scp_wfg_lbfgsb(Z, W, A, Znormsqr, beta)
% SCP_WFG_LBFGSB Computes function and gradient of the CP function for data with
% missing entries. SCP_WFG_LBFGSB also has the option of imposing sparsity on the 
% weights of rank-one components.
%
%   [f,G] = scp_wfg_lbfgsb(Z, W, A, Znormsqr, beta) 
%
...
%                    + 0.5* beta *sum(A{end})
%            where A{end} is constrained to be nonnegative.
...
%  
% See also SCP_FG_LBFGSB, ACMTF_FG_LBFGSB
%
...
f = f + 0.5 *beta*sum(Lambda); %changed for lbfgsb!
% eps = 1e-8;
% for r=1:R
...
G{N+1} = G{N+1} + 0.5*beta*ones(R,1); %changed for lbfgsb!
% for r=1:R
